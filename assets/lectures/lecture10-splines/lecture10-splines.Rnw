...
\begin{frame}[fragile]{Time series analysis}
...
<<flu-plot, fig.height=3.5, message=FALSE, warning=FALSE, echo=FALSE>>=
ggplot(usflu, aes(x=date, y=ili_weighted)) + geom_point()
@
...
$$\mathbb E [y_t] = \beta_0 + \beta_1\cdot \sin(2\pi t) + \cos(1\pi t)$$
\scriptsize
<<flu-trig-model, fig.height=3.5, message=FALSE, warning=FALSE>>=
usflu <- usflu[!is.na(usflu$dec_date),] ## sloppy: removing one "week 53"
...
\begin{frame}[fragile]{Fit a smoothing splines model}
\scriptsize
