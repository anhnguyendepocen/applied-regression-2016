...
<<donnerPlot, echo=TRUE, fig.height=3.8>>=
(p <- ggplot(birds, aes(x=CD, y=as.numeric(LC=="LungCancer")*1)) + 
     geom_jitter(height=0) + geom_smooth(method="lm", se=FALSE) + 
     ylab("Lung Cancer") + xlab("Cigarettes per day") + 
...
x <- seq(-6, 6, by=.1)
y <- exp(x)/(1+exp(x))
qplot(x, y, geom="line") + ggtitle("Logistic function") + xlab("x") + ylab("p") + annotate("text", x=-4, y=.75, label="p == frac(e^x,1+e^x)", parse=TRUE)
@
...
\bi
        ...
        \myitem Do we have the same power to infer smooth splines in non-Gaussian GLMs?
