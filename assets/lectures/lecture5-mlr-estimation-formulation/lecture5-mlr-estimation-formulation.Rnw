...
\begin{frame}[t]{Geometry of least squares (in 3D)}


\begin{figure}[t]
    \includegraphics[width=\textwidth]{ls-geometry-plot.png}  
\end{figure}
\begin{figure}[t]
    \includegraphics[width=.6\textwidth]{ls-geometry-model.png}  
\end{figure}

{\footnotesize * Image credits: Simon Wood, {\em Core Statistics}, Figure 7.1.}
