...
\begin{exercise}
Refer to the link provided for the data description. What was this data originally collected for? What were the authors trying to investigate? 
\end{exercise}
...
\begin{exercise}
Which variable would you expect to have the highest p-value in this model? Why? \textit{Hint:} Think about which variable you would expect to have no or little association with weight.
\end{exercise}
